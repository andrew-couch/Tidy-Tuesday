...
fighter_hist <- read_csv(here("Data/fight_data.csv"))
future_cards <- read_csv(here("Data/future_card_predictions.csv"))
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "prediction_tab",
              fluidRow(box(uiOutput("weight_class_selector"))),
...
server <- function(input, output) {
  ...
  output$fighter_selector <- renderUI({
    selectInput(inputId = "v_fighter", label = "Fighter", choices = fighter_df %>% filter(weight_class == input$v_weight_class) %>% 
                  select(fighter) %>% arrange(fighter) %>% distinct())
  })
