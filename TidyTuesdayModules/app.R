    ...
    moduleServer(id, function(input, output, session){
        output$penguin_pca <- renderPlot({
            ...
            pca_data <- recipe(species~., data = df) %>% 
                ...
                prep() %>% 
                juice() %>% 
                mutate(color = if_else(species == "Adelie", "Adelie", "Not Adelie"))
    ...
    moduleServer(id, function(input, output, session){        
        output$penguin_pairs <- renderPlot({
            ...
            df %>% 
                ...
                mutate(species = if_else(tolower(species) == type, type, "Other")) %>%
    ...
    dashboardBody(
        tabItems(
            tabItem(tabName = "adelie",
                    fluidRow(box(DT::dataTableOutput("adelie_table")), box(plotOutput("adelie_correlation"))),
                    fluidRow(box(plotOutput("adelie_pca")), box(plotOutput("adelie_pairs")))),
...
server <- function(input, output) { 
    ...
    # Adelie Tab --------------------------------------------------------------
    output$adelie_table <- DT::renderDataTable({
        DT::datatable(
            df %>% 
    ...
    output$adelie_correlation <- renderPlot({
        df %>% 
            filter(species == "Adelie") %>% 
            select_if(is.numeric) %>% 
            cor() %>% 
    ...
    output$adelie_pairs <- renderPlot({
        df %>% 
            select(species, bill_length_mm:body_mass_g) %>% 
            mutate(species = if_else(species == "Adelie", "Adelie", "Not Adelie")) %>%
            GGally::ggpairs(aes(color = species), progress = FALSE)
    })
    # Chinstrap ---------------------------------------------------------------
